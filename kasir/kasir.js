...
// ==========================================
// 6. FUNGSI CETAK NOTA (STRUK THERMAL 58MM)
// ==========================================
function cetakNota(items, total) {
    // Meminta Kasir memasukkan uang bayar (Bisa dikosongkan/Cancel jika uang pas)
    let inputUang = prompt(`TOTAL: Rp ${total.toLocaleString('id-ID')}\n\nBerapa Uang Tunai yang diterima dari pelanggan? (Kosongkan jika uang pas)`);
    let uangBayar = Number(inputUang) || total;
    ...
    items.forEach(item => {
        strukHTML += `
            <tr>
                <td colspan="3" class="text-bold">${item.Nama}</td>
            </tr>
            <tr>
    ...
    strukHTML += `
        ...
        <table class="struk-table text-bold">
            ...
            <tr>
                <td>TUNAI</td>
                <td class="text-right">Rp ${uangBayar.toLocaleString('id-ID')}</td>
            </tr>
            <tr>
                <td>KEMBALI</td>
                <td class="text-right">Rp ${kembalian.toLocaleString('id-ID')}</td>
            </tr>
        </table>
        <div class="struk-divider"></div>
